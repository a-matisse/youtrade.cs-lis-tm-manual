...
- [КАК РАБОТАЕТ БОТ АВТОПРОДАЖИ](#как-работает-бот-автопродажи)
    ...
    - [Просмотр списка предметов AutoSell](#просмотр-списка-предметов-autosell)
...
После подтверждения доступа заполните обязательные поля, чтобы бот корректно работал. Выполните команду:
`/help_update_autobuy`
...
**Минимальная рентабельность (minProfit)** — Этот параметр определяет минимальный процент маржи, при котором покупка
будет считаться выгодной. **minProfit** зависит от разницы между покупной ценой и ценой продажи с учетом всех комиссий. Чем выше значение, тем выше потенциальная прибыль, но тем меньше предложений бот будет
рассматривать.
...
**Минимальный процент AutoBuy (maxAutoBuyPercent)** — Указывает минимальное соотношение цены на площадке покупки к цене автопокупки на площадке продажи. Чем ниже это значение, тем более рискованные предложения может рассмотреть бот. **НЕ РЕКОМЕНДУЕТСЯ УСТАНАВЛИВАТЬ ЗНАЧЕНИЕ МЕНЬШЕ 97% И БОЛЬШЕ 102%**, если вы не преследуете цель ловли отдельных, определенных редких коэффициентов или предметов. Чем ниже значение, тем более выгодные позиции будет выбирать бот, но тем меньше предложений бот будет рассматривать.
...
**Минимальное время удержания (minDaysHold)** — задаёт минимальное количество дней, на которое площадка покупки будет задерживать предмет перед получением.  
...
**Максимальное количество дней удержания (maxDaysHold)** — Задает предельное количество дней задержки перед получением. Этот фильтр позволяет выбирать такие предметы, что они быстрее попадут вам в инвентарь. Допустимое значение — от **minDaysHold** до **8** дней.
...
### Управление продажами через Excel-шаблоны
